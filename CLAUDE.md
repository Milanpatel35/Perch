...
| 2026-09-22 | feature/2-5-camera | **Phase 2.5 (first half) — the Camera.** `CameraPresentation`, `CameraSelection` and `PreCallCheck` in Core; `CameraSession`, `CameraPinPanel`, the service, the snapshot half, the views and the settings pane in the module. **The privacy promises hold by construction, not by discipline:** a running preview has *no output attached to the session at all* — only an `AVCaptureVideoPreviewLayer`, which renders from the device and hands Perch nothing — so there is no frame to keep and none to write. Closing **removes the session's inputs** rather than only stopping it; stopping alone leaves the device held and the green light on. TC-CAM-009 runs the whole lifecycle five times to prove nothing but the preview opens a device. The island collapsing is what closes the camera, and it is the island that says so rather than the view disappearing — a view is torn down whenever SwiftUI feels like it, and "within 500ms" cannot hang on that. The pinned pill is the one window in the app that is not `IslandPanel`, and §9's exception is recorded in its own doc comment. The pre-call check is wired calendar → camera in `PerchModuleRegistry`; with the calendar off nothing fires, which is TC-CAM-015 for free. 416 tests. | **Phase 2.5 (second half): system stats.** |

| 2026-09-22 | feature/2-5-system-stats | **Phase 2.5 complete — the system monitor.** `SystemSnapshot`, `SamplerPolicy`, `StatHistory` and `StatAlerts` in Core; `SystemReader` (Mach) split from its IOKit and network halves, the service, both activities, the views and the settings pane in the module. **The performance trap `CLAUDE.md` §4 warns about is closed by construction:** the sampler's lifetime belongs to the *view* (`beginSampling`/`endSampling` on appear and disappear), and `SamplerPolicy.interval(for:)` returns **`nil`** rather than a long number so a caller cannot read "do not sample" as a default. Demands are counted rather than flagged, because both views are up during the expand animation. Every number comes from a documented syscall or a public registry read — no stats library, per §9. Three rows are refusals and each is written down: temperature needs `IOHIDEventSystemClient` (private), GPU load does not exist on Intel, and per-volume disk throughput would attribute to devices rather than volumes. An unreadable sensor reads `nil`, never zero, so a temperature alert cannot report "fine" for ever. The public-IP readout is the one network request in the app, off, asked once, in a file by itself. 452 tests. | **Phase 2.6: weather, windows, Shortcuts, hide-the-notch.** |
